...
// get all pets
export const fetchAllpetsAction = createAsyncThunk(
  "all/pet/fetch",
  async (payload, { rejectWithValue, getState, dispatch }) => {
    //get user token from store
    const userToken = getState()?.users?.userAuth?.token;
    ...
    try {
      ...
      const { data } = await axios.get(`${BaseURL}/pets/all`, config);
      return data;
    } catch (error) {
      if (!error?.response) {
        throw error;
...
// edit Pet
export const editPetsAction = createAsyncThunk(
  "pets/update",
  async (payload, { rejectWithValue, getState, dispatch }) => {
    //get user token from store
...
const PetsSlices = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //handle pending state
    builder.addCase(fetchAllpetsAction.pending, (state, action) => {
      state.petsLoading = true;
    });

    //hande success state
    builder.addCase(fetchAllpetsAction.fulfilled, (state, action) => {
      state.allPets = action?.payload;
      state.petsLoading = false;
    });
    //hande rejected state

    builder.addCase(fetchAllpetsAction.rejected, (state, action) => {
      state.petsLoading = false;
      state.petAppErr = action?.payload?.msg;
      state.petServerErr = action?.error?.msg;
    });
